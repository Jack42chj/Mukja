...
문제: 현재 위치를 지정하기 위해 지도에서 화면 가운데 핀을 고정해 이동한 위치로 현재 위치를 설정하도록 구현했다. 이 부분에서 화면을 움직이면 해당 좌표에 대한 위도와 경도의 대한 데이터를 계속 업데이트 하기 때문에 렌더링 발생이 지속적으로 일어나는 문제가 있었다.

대안: 완벽하게 해결할 수는 없지만 그래도 최선의 선택으로 React에서 지원하는 hook 'useMemo'를 활용해 메모이제이션 기법으로 데이터 패칭의 빈도수를 낮췄다.

#### 📍 가까운 거리순의 데이터를 제공하기 위한 함수 구현

문제: 위치를 설정하고 해당 위치를 기반으로 가까운 거리의 데이터를 제공하려 하였다. 찾아보니 supabase에서 제공하는 PosGIS를 활용하면 이를 구현 가능하도록 되어있어 사용하려 했으나 React에서 호출할 수 있는 supabase 메서드가 제한이 되어있어 사용하지 못하였다.
...
프로젝트 초기에는 데이터 수집 방식을 크롤링으로 구현하려 하였다. 우선 서울에 있는 맛집을 타켓으로 조사한 결과 대략 18,000개의 맛집이라고 알려져 있다. 결국 이를 확인하고 검증하기 위해 직접 찾아야 수집을 하는 과정에서는 이미 사라진 집이나 폐업한 식당이 너무 많이 존재하였다. 이를 검수하는 작업을 하면서 DB를 구축하니 시간이 너무 많이 소모된다.

문제2: 이미지 데이터 수집

맛집이라는 주제로 프로젝트를 진행하니 사진에 대한 신뢰도가 높다고 생각하였다. 최근 배달앱 리뷰를 봐도 일반 글로 써진 리뷰보다 사진 리뷰의 관심이 많고 이를 활용한 마케팅이 중요하다고 생각했다. 그러나 사진과 관련해서는 저작관 문제가 걸려있어 추후에 문제가 될 것이라 생각해 이를 수집하는 것도 쉽지 않다고 판단했다. 이를 해결하기 위해서는 직접 수집하거는 방법밖에 없다고 생각한다.

위와 같은 문제들로 결국 프로젝트는 완성시키기 힘들다고 판단하여 UI 및 기능 구현은 끝냈지만 아쉽게도 데이터 수집 문제로 진행을 중단하였다.
